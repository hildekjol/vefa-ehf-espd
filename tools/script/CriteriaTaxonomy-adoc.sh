...
for id in $(cat $PROJECT/codelist/CriteriaTaxonomy.xml | grep "<Criterion id=" | cut -d '"' -f 2); do
  # echo "Crit: $id"
  echo "
...
for id in $(cat $PROJECT/codelist/CriteriaTaxonomy.xml | grep "<RequirementGroup id=" | cut -d '"' -f 2); do
  # echo "ReqGr: $id"
  echo "
...
for id in $(cat $PROJECT/codelist/CriteriaTaxonomy.xml | grep "<Requirement id=" | cut -d '"' -f 2); do
  # echo "Req: $id"
  echo "
